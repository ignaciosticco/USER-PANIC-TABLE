...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_fgmodified.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTableFgmodified::PairPanicTableFgmodified(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableFgmodified::~PairPanicTableFgmodified()
{
  if (copymode) return;
...
void PairPanicTableFgmodified::compute(int eflag, int vflag)
{
...
void PairPanicTableFgmodified::allocate()
{
  allocated = 1;
...
void PairPanicTableFgmodified::settings(int narg, char **arg)
{

  if (narg < 5) error->all(FLERR,"Illegal pair_style command");	// Sticco 3 April 2019
  ...
  int iarg = 5;					// Sticco 3 April 2019	
  while (iarg < narg) {
...
void PairPanicTableFgmodified::coeff(int narg, char **arg)
{
...
double PairPanicTableFgmodified::init_one(int i, int j)
{
...
void PairPanicTableFgmodified::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableFgmodified::bcast_table(Table *tb)
{
...
void PairPanicTableFgmodified::spline_table(Table *tb)
{
...
void PairPanicTableFgmodified::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableFgmodified::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableFgmodified::null_table(Table *tb)
{
...
void PairPanicTableFgmodified::free_table(Table *tb)
{
...
void PairPanicTableFgmodified::spline(double *x, double *y, int n,
...
double PairPanicTableFgmodified::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableFgmodified::write_restart(FILE *fp)
{
...
void PairPanicTableFgmodified::read_restart(FILE *fp)
{
...
void PairPanicTableFgmodified::write_restart_settings(FILE *fp)
{
...
void PairPanicTableFgmodified::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableFgmodified::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableFgmodified::extract(const char *str, int &dim)
{
...
void PairPanicTableFgmodified::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableFgmodified::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb, double rad){
...
void PairPanicTableFgmodified::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
...
void PairPanicTableFgmodified::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
