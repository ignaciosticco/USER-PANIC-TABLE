...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_kcomp.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
#define KCOMP    120000.0  // Sticco 21 Ago 2019 

/* ---------------------------------------------------------------------- */

PairPanicTableKcomp::PairPanicTableKcomp(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableKcomp::~PairPanicTableKcomp()
{
  if (copymode) return;
...
void PairPanicTableKcomp::compute(int eflag, int vflag)
{
...
void PairPanicTableKcomp::allocate()
{
  allocated = 1;
...
void PairPanicTableKcomp::settings(int narg, char **arg)
{
...
void PairPanicTableKcomp::coeff(int narg, char **arg)
{
...
double PairPanicTableKcomp::init_one(int i, int j)
{
...
void PairPanicTableKcomp::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableKcomp::bcast_table(Table *tb)
{
...
void PairPanicTableKcomp::spline_table(Table *tb)
{
...
void PairPanicTableKcomp::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableKcomp::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableKcomp::null_table(Table *tb)
{
...
void PairPanicTableKcomp::free_table(Table *tb)
{
...
void PairPanicTableKcomp::spline(double *x, double *y, int n,
...
double PairPanicTableKcomp::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableKcomp::write_restart(FILE *fp)
{
...
void PairPanicTableKcomp::read_restart(FILE *fp)
{
...
void PairPanicTableKcomp::write_restart_settings(FILE *fp)
{
...
void PairPanicTableKcomp::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableKcomp::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableKcomp::extract(const char *str, int &dim)
{
...
void PairPanicTableKcomp::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableKcomp::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb, double rad){
        ...
        if (c1 == 0.0 && x < wp2 && x > wp1 ){          //pared horizontal
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    force_wall[1] += -KAPPA*gpair*vy;  // Sticco Cornes 29-Jun 2018
                }		
            }		
        }
...
void PairPanicTableKcomp::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
...
void PairPanicTableKcomp::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
