...
/* ----------------------------------------------------------------------
  Este modulo tiene como parametros las constantes del SFM basico:
  vd,tau,A,B,Kcomp,Kappa,Kappawall
------------------------------------------------------------------------- */
...
#include <stdlib.h>
#include <string.h>
#include "pair_panic_table_custom.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTableCustom::PairPanicTableCustom(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTableCustom::~PairPanicTableCustom()
{
  if (copymode) return;
...
void PairPanicTableCustom::compute(int eflag, int vflag)
{
  ...
  Table  *tb;
  double granular_factor, socialx, socialy,compresion_factor; // Sticco 1 Ago 2018
  ...
  double **f = atom->f;
  double *mass = atom->rmass;             // Sticco 4 Nov 2019
  ...
  double m = mass[0]; // Warning: all particles same mass
  double MOT = m/TAU; // Sticco 4 Nov 2019
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        compresion_factor = KCOMP*gpair/sqrt(rsq);               // Sticco 1 Ago 2019
        socialx = delx*(fpair+compresion_factor);                // Sticco 1 Ago 2019
        socialy = dely*(fpair+compresion_factor);                // Sticco 1 Ago 2019
    ...
    compute_desiredforce(xtmp, ytmp, vxtmp, vytmp, force_desired,vd,tb,MOT); // Sticco 18 Jun 2018
...
void PairPanicTableCustom::allocate()
{
  allocated = 1;
...
void PairPanicTableCustom::settings(int narg, char **arg)
{
  ...
  TAU = atof(arg[3]);      // Sticco 4 Nov 2019
  A = atof(arg[4]);        // Sticco 4 Nov 2019  
  B = atof(arg[5]);        // Sticco 4 Nov 2019  
  KCOMP = atof(arg[6]);    // Sticco 4 Nov 2019  
  KAPPA = atof(arg[7]);     // Sticco 4 Nov 2019 
  KAPPAWALL = atof(arg[8]); // Sticco 4 Nov 2019  
  int iarg = 9;
  while (iarg < narg) {
...
void PairPanicTableCustom::coeff(int narg, char **arg)
{
...
double PairPanicTableCustom::init_one(int i, int j)
{
...
void PairPanicTableCustom::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTableCustom::bcast_table(Table *tb)
{
...
void PairPanicTableCustom::spline_table(Table *tb)
{
...
void PairPanicTableCustom::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTableCustom::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTableCustom::null_table(Table *tb)
{
...
void PairPanicTableCustom::free_table(Table *tb)
{
...
void PairPanicTableCustom::spline(double *x, double *y, int n,
...
double PairPanicTableCustom::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTableCustom::write_restart(FILE *fp)
{
...
void PairPanicTableCustom::read_restart(FILE *fp)
{
...
void PairPanicTableCustom::write_restart_settings(FILE *fp)
{
...
void PairPanicTableCustom::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTableCustom::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTableCustom::extract(const char *str, int &dim)
{
...
void PairPanicTableCustom::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTableCustom::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb, double rad){
    ...
    if (c1*c2 == 0.0){
        ...
        if (c1 == 0.0 && x < wp2 && x > wp1 ){          //pared horizontal
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    force_wall[0] += -KAPPAWALL*gpair*vx; // Sticco 4 Nov 2019
                    force_wall[1] += dely*(fpair+gpair*KCOMP/fabs(dely));     // Sticco 27 Ago 2019 	
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    if (rad2>rsq) gpair = rad-fabs(delx);    // Sticco Cornes 29-Jun 2018
                    force_wall[0] += delx*(fpair+gpair*KCOMP/fabs(delx));     // Sticco 27 Ago 2019
                    force_wall[1] += -KAPPAWALL*gpair*vy;  // Sticco 4 Nov 2019
                }		
            }		
        }
        ...
        if (rsq < RCUT2) {
            ...
            if (y < yup && y > ydown){ 
                ...
                force_wall[0] += delx*fpair-KAPPAWALL*gpair*(dely*vx-delx*vy)*dely/cociente; // Sticco 4 Nov 2019
                force_wall[1] += dely*fpair+KAPPAWALL*gpair*(dely*vx-delx*vy)*delx/cociente; // Sticco 4 Nov 2019
            }	
        }
...
void PairPanicTableCustom::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb,double MOT){
...
void PairPanicTableCustom::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
