...
/* ----------------------------------------------------------------------
   Este codigo esta hecho a partir de pair_panic_table_fgfd_modified
   Es igual a este pero permite simular obstaculos tipo pilar 
   (lo lee de la tabla)
------------------------------------------------------------------------- */
...
#include <cstdlib>
#include <ctime>
#include "pair_panic_table_pilar.h"
#include "atom.h"
#include "force.h"
#include "comm.h"
...
PairPanicTablePilar::PairPanicTablePilar(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
PairPanicTablePilar::~PairPanicTablePilar()
{
  if (copymode) return;
...
void PairPanicTablePilar::compute(int eflag, int vflag)
{
  ...
  double force_pilar[2];            // Sticco 9 April 2019
  ...
  srand (static_cast <unsigned> (time(0))); // Sticco 4 April 2019
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]){
        ...
        granular_factor = factor_fgpedestrian*KAPPA*gpair*(dely*delvx-delx*delvy)/rsq; // Sticco 3 April 2019
    ...
    compute_pilarforce(xtmp, ytmp, vxtmp, vytmp, force_pilar, tb, radius[i]); // Sticco 9 April 2018
    f[i][0] += force_pilar[0];    
    f[i][1] += force_pilar[1];    
  }
...
void PairPanicTablePilar::allocate()
{
  allocated = 1;
...
void PairPanicTablePilar::settings(int narg, char **arg)
{

  if (narg < 6) error->all(FLERR,"Illegal pair_style command");	// Sticco 4 April 2019
  ...
  factor_fgpedestrian = atof(arg[3]);   // Sticco 3 April 2019
  factor_fgwall = atof(arg[4]);        // Sticco 3 April 2019
  noise_amp = atof(arg[5]);           // Sticco 3 April 2019
  int iarg = 6;				               	// Sticco 4 April 2019	
  while (iarg < narg) {
...
void PairPanicTablePilar::coeff(int narg, char **arg)
{
...
double PairPanicTablePilar::init_one(int i, int j)
{
...
void PairPanicTablePilar::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTablePilar::bcast_table(Table *tb)
{
  ...
  MPI_Bcast(&tb->pilar_ninput,1,MPI_INT,0,world);      // Sticco 9 April 2019 
  ...
  if (me > 0) {
    ...
    memory->create(tb->p1,tb->pilar_ninput,"pair:p1");    
    memory->create(tb->p2,tb->pilar_ninput,"pair:p2");
    memory->create(tb->p3,tb->pilar_ninput,"pair:p3");
    memory->create(tb->p4,tb->pilar_ninput,"pair:p4");
    //----------------------------------------------------
  }
  ...
  MPI_Bcast(tb->p1,tb->pilar_ninput,MPI_DOUBLE,0,world);
  MPI_Bcast(tb->p2,tb->pilar_ninput,MPI_DOUBLE,0,world);
  MPI_Bcast(tb->p3,tb->pilar_ninput,MPI_DOUBLE,0,world);
  MPI_Bcast(tb->p4,tb->pilar_ninput,MPI_DOUBLE,0,world);
...
void PairPanicTablePilar::spline_table(Table *tb)
{
...
void PairPanicTablePilar::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTablePilar::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTablePilar::null_table(Table *tb)
{
...
void PairPanicTablePilar::free_table(Table *tb)
{
  ...
  memory->destroy(tb->p1);
  memory->destroy(tb->p2);
  memory->destroy(tb->p3);
  memory->destroy(tb->p4);
  //--------------------------------------------------
}
...
void PairPanicTablePilar::spline(double *x, double *y, int n,
...
double PairPanicTablePilar::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTablePilar::write_restart(FILE *fp)
{
...
void PairPanicTablePilar::read_restart(FILE *fp)
{
...
void PairPanicTablePilar::write_restart_settings(FILE *fp)
{
...
void PairPanicTablePilar::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTablePilar::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTablePilar::extract(const char *str, int &dim)
{
...
void PairPanicTablePilar::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
  ...
  if (sscanf(line+1,"%s %d",word,&n)==2){
         if (strcmp(word,"W") == 0){ 
           ...
           for (i=0;i<n;i++){
          ...
          else if (strcmp(word,"P") == 0){                // Sticco 9 April 2019
            tb->pilar_ninput = n;                         // Sticco 9 April 2019
            memory->create(tb->p1,tb->pilar_ninput,"pair:p1");
            memory->create(tb->p2,tb->pilar_ninput,"pair:p2");
            memory->create(tb->p3,tb->pilar_ninput,"pair:p3");
            memory->create(tb->p4,tb->pilar_ninput,"pair:p4");
            for (i=0;i<n;i++){
                   fgets(word,MAXLINE,fp);
                   sscanf(word+1,"%lg %lg %lg %lg",&pp1,&pp2,&pp3,&pp4);
                   tb->p1[i] = pp1;
                   tb->p2[i] = pp2;
                   tb->p3[i] = pp3;
                   tb->p4[i] = pp4;
...
void PairPanicTablePilar::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb, double rad){
    ...
    if (c1*c2 == 0.0){
        ...
        if (c1 == 0.0 && x < wp2 && x > wp1 ){          //pared horizontal
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    force_wall[0] += -factor_fgwall*KAPPA*gpair*vx; // Sticco 3 April 2019
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    force_wall[1] += -factor_fgwall*KAPPA*gpair*vy; // Sticco 3 April 2019
                }   
            }   
        }
        ...
        if (rsq < RCUT2) {
            ...
            if (y < yup && y > ydown){ 
                ...
                force_wall[0] += delx*fpair-factor_fgwall*KAPPA*gpair*(dely*vx-delx*vy)*dely/cociente; // Sticco 3 April 2019
                force_wall[1] += dely*fpair+factor_fgwall*KAPPA*gpair*(dely*vx-delx*vy)*delx/cociente; // Sticco 3 April 2019
            } 
        }
...
void PairPanicTablePilar::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
  double xi,xf,yi,yf,rsq,dx,dy,dx_min,dy_min,r,rinv,nx,ny,noise;
  ...
  noise = -noise_amp + static_cast <double> (rand()) /( static_cast <double> (RAND_MAX/2.0))*noise_amp; // Sticco 4 April 2019                 
  int j=0;
    while(j< tb->opening_ninput){
      ...
      if(orientation == 0){                // Vertical orientation
          ...
          if (dx*pointing > 0){            //significa que no paso por el opening
            ...
            if (y >= yi && y <= yf){
                rsq = dx*dx;
                dy = 0.0+noise;           // Sticco 4 April 2019
                ubication = 0;
            }
            else if (y < yi){
                dy = yi+noise - y;       // Sticco 4 April 2019
                rsq = dx*dx + dy*dy;      
                ubication = 1;
            }
            else {
                dy = yf+noise - y;       // Sticco 4 April 2019
                rsq = dx*dx + dy*dy;       
                ubication = 2;
            }
          }
          ...
          if (dy*pointing < 0){            //No paso por el opening
            if(x >= xi && x <= xf){
                dx = 0.0+noise;            // Sticco 4 April 2019   
                rsq = dy*dy;
                ubication = 0;
            }
            else if (xi < x){
                dx = xi+noise - x;         // Sticco 4 April 2019
                rsq = dx*dx + dy*dy;
                ubication = 1;
            }
            else{
                dx = xf+noise - x;         // Sticco 4 April 2019
                rsq = dx*dx + dy*dy;
                ubication = 2;
            }  
          }
...
void PairPanicTablePilar::search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb){
...
// Sticco 12 April 2019 //
void PairPanicTablePilar::compute_pilarforce(double x, double y, double vx, double vy, double force_pilar[],Table *tb, double rad){
  
  double x_center_pilar,y_center_pilar,z_center_pilar,radius_pilar;
  double x_edge_pilar,y_edge_pilar,z_edge_pilar,den_versor;
  double delx,dely,delz,delvx,delvy,rsq,fpair,gpair,rad2; 
  double fraction,value;
  double granular_factor, socialx, socialy; 
  double cexp1;
  char   estr[128];
  ...
  for(int j = 0; j < tb->pilar_ninput; ++j){
    x_center_pilar = tb->p1[j]; 
    y_center_pilar = tb->p2[j];
    z_center_pilar = tb->p3[j];
    radius_pilar = tb->p4[j];
    
    den_versor = sqrt((x-x_center_pilar)*(x-x_center_pilar)+(y-y_center_pilar)*(y-y_center_pilar));
    x_edge_pilar = x_center_pilar + radius_pilar*(x-x_center_pilar)/den_versor;
    y_edge_pilar = y_center_pilar + radius_pilar*(y-y_center_pilar)/den_versor;
    ...
    rsq = delx*delx + dely*dely;
      ...
      if (rsq < RCUT2){
         if (tabstyle == LINEAR){
          ...
          value = tb->f[itable] + fraction*tb->df[itable];
          fpair = value;                                              
          value = tb->rsqt[itable] + fraction*tb->drsqt[itable];       
          gpair = 0.0;
          rad2 = rad * rad;                                         
          if (rad2>rsq) gpair = rad-fabs(rsq); 
          /////////////////////////////////////////////////////////////////
          rsq = rad2;  // rsq is now rad2 to find exp for pdestrian radius               
          itable = static_cast<int> ((rsq - tb->innersq) * tb->invdelta);   
          fraction = (rsq - tb->rsq[itable]) * tb->invdelta;                
          value = tb->f[itable] + fraction*tb->df[itable];                  
          cexp1 = 1.0/(value * sqrt(rsq));                                  
          fpair = A*cexp1* fpair;                                          
          ///////////////////////////////////////////////////////////////// 

          rsq = delx*delx + dely*dely;                              
          granular_factor = factor_fgwall*KAPPA*gpair*(dely*delvx-delx*delvy)/rsq; 
          socialx = delx*fpair;                                      
          socialy = dely*fpair;                                      

          force_pilar[0] = socialx - granular_factor*dely;
  		  force_pilar[1] = socialy + granular_factor*delx;
