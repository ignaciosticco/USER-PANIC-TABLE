...
#define CEXP1    3616084.8
#define CEXP2    85042.1
#define RAD2     0.09
#define MOT      140.0

/* ---------------------------------------------------------------------- */

PairPanicTable::PairPanicTable(LAMMPS *lmp) : Pair(lmp)
{
  ntables = 0;
  tables = NULL;
}
...
void PairPanicTable::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (tabstyle == LOOKUP) {
          ...
          fpair = CEXP1* factor_lj * value;		                             // Sticco 18 Jun 2018
    ...
    compute_wallforce(xtmp, ytmp, vxtmp, vytmp, force_wall, tb);    // Sticco 18 Jun 2018
...
void PairPanicTable::settings(int narg, char **arg)
{
...
void PairPanicTable::coeff(int narg, char **arg)
{
...
double PairPanicTable::init_one(int i, int j)
{
...
void PairPanicTable::read_table(Table *tb, char *file, char *keyword)
{
  char line[MAXLINE];
...
void PairPanicTable::bcast_table(Table *tb)
{
...
void PairPanicTable::spline_table(Table *tb)
{
...
void PairPanicTable::param_extract(Table *tb, char *line)
{
  tb->ninput = 0;
  tb->rflag = NONE;
  tb->fpflag = 0;
...
void PairPanicTable::compute_table(Table *tb)
{
  int tlm1 = tablength-1;
...
void PairPanicTable::null_table(Table *tb)
{
...
void PairPanicTable::free_table(Table *tb)
{
...
void PairPanicTable::spline(double *x, double *y, int n,
...
double PairPanicTable::splint(double *xa, double *ya, double *y2a, int n, double x)
{
  int klo,khi,k;
  double h,b,a,y;
...
void PairPanicTable::write_restart(FILE *fp)
{
...
void PairPanicTable::read_restart(FILE *fp)
{
...
void PairPanicTable::write_restart_settings(FILE *fp)
{
...
void PairPanicTable::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairPanicTable::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairPanicTable::extract(const char *str, int &dim)
{
...
void PairPanicTable::wall_extract(Table *tb, char *line,FILE *fp)
{
  int  i,n;
  char word[MAXLINE];
...
void PairPanicTable::compute_wallforce(double x, double y, double vx, double vy, double force_wall[], Table *tb){

  double rsq,numerator,denominator,xp,yp,c1,c2,c3,c4,c5,c6,c7,c8,c9,r;
        ...
        if (c1 == 0.0 && x < wp2 && x > wp1 ){          //pared horizontal
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    fpair = CEXP2* value;
                    gpair = 0.0;
                    if (RAD2>rsq) gpair = RAD-fabs(dely);
            ...
            if (rsq < RCUT2) {
                if (tabstyle == LINEAR) {
                    ...
                    fpair = CEXP2 * value;
                    gpair = 0.0;
                    if (RAD2>rsq) gpair = RAD-fabs(delx);
        ...
        if (rsq < RCUT2) {
            ...
            if (y < yup && y > ydown){ 
                if (tabstyle == LINEAR) {
                    ...
                    fpair = CEXP2* value;
                    gpair = 0.0;
                    if (RAD2>rsq) gpair = RAD-sqrt(rsq);
                }
                force_wall[0] += delx*fpair-KAPPA*gpair*(dely*vx-delx*vy)*dely/rsq; // Sticco 18 Jun 2018
                force_wall[1] += dely*fpair+KAPPA*gpair*(dely*vx-delx*vy)*delx/rsq; // Sticco 18 Jun 2018
            }	
        }
...
void PairPanicTable::compute_desiredforce(double x, double y, double vx, double vy, double* force_desired, double vd, Table *tb){
