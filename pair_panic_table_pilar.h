...
PairStyle(panic/table/pilar,PairPanicTablePilar)

#else

#ifndef LMP_PAIR_PANIC_TABLE_PILAR_H
#define LMP_PAIR_PANIC_TABLE_PILAR_H

#include "pair.h"

namespace LAMMPS_NS {

class PairPanicTablePilar : public Pair {
 public:
  PairPanicTablePilar(class LAMMPS *);
  virtual ~PairPanicTablePilar();
 ...
 protected:
  ...
  struct Table {
    int    wall_ninput,opening_ninput,pilar_ninput;         // Sticco 9 April 2019
    ...
    double *p1,*p2,*p3,*p4;                            // Sticco 9 April 2019
  };
  int ntables;
  Table *tables;

  int    arg_wall = 14;                                      // Sticco 18 Jun 2018
  double vd,factor_fgpedestrian,factor_fgwall,noise_amp;      // Sticco 4 April 2019
  int    num_pasos = 0;                                       // Sticco Cornes 22 Jun

  double vector_vd_x[100000];                                 // Sticco Cornes 22 Jun
  double vector_vd_y[100000];                                 // Sticco Cornes 22 Jun
  ...
  void compute_wallforce(double, double, double, double, double force_wall[], Table *,double rad);  // Sticco 18 Jun 2018
  ...
  void search_near_opening(int i, double vector_vd_x[], double vector_vd_y[], Table *tb);
  void compute_pilarforce(double x, double y, double vx, double vy, double force_pilar[],Table *tb, double rad);   // Sticco 9 April 2019 
